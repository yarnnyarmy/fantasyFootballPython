...
from footballPlayers.Rb import Rb

allRbs = Rb()


def get_all_rbs():
    url = 'https://www.pro-football-reference.com/fantasy/RB-fantasy-matchups.htm'
    ...
    player_dict = {'name':[], 'link':[], 'team':[], 'injury':[], 'gamesPlayed':[], 'gamesStarted':[], 'snaps':[],
                   ...
                   'fantasy_points_per_game':[], 'draftkings_points_per_game,':[], 'fanduel_points_per_game':[],
                  'opp':[], 'ranker':[], 'opp_fantasy_points_per_game':[], 'opp_draftkings_points_per_game':[],
                   'opp_fanduel_points_per_game':[],'fantasy_points_proj_rank':[], 'draftkings_points_proj_rank':[],
    ...
    cursor.execute('DROP TABLE IF EXISTS dbo.RBFantasy')

    cursor.execute('''CREATE TABLE [dbo].[RBFantasy](
    	[RbId] [int] IDENTITY(1,1) NOT NULL,
    	[nameRb] [nvarchar](255) NOT NULL,
    ...
    insert_query = '''INSERT INTO RBFantasy(nameRb, link, team, injury, gamesPlayed, gamesStarted, snaps,
    rush_att, rush_yds, rush_td, pass_targets, pass_recep, receiving_yds, receiving_td, fantasy_points,
...
def csv_to_list_rb():
    ...
    for i in df.values:
        ...
        new_arr.append(arr1)
    rb_dict = {'name': [], 'id': [], 'position': [], 'salary': [], 'team': [], 'draftkings_points': []}
    for i in new_arr:
        if i[2] == 'RB/FLEX':
            rb = Rb()
            rb.set_name(i[0])
            ...
            rb.set_salary(i[3])
            rb.set_team(i[4])
            rb.set_draftkings_points(i[5])
            all_rbs.append(rb)
            rb_dict['name'].append(rb.get_name())
            rb_dict['id'].append(rb.get_id())
            rb_dict['position'].append(rb.get_position())
            rb_dict['team'].append(rb.get_team())
            rb_dict['draftkings_points'].append(rb.get_draftkings_points())
            rb_dict['salary'].append(rb.get_salary())

    ef = pd.DataFrame(rb_dict)
